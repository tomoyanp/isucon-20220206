...
func postApiV1ReservationHome(c echo.Context) error {
	...
	getReservationHomeQuery := `SELECT rh.id FROM isubnb.home h JOIN isubnb.reservation_home rh ON h.id=rh.home_id WHERE h.id = ? AND ? <= rh.date AND rh.date < ?`
	err = db.Select(&reservationHomeIdList, getReservationHomeQuery, request.HomeId, request.StartDate, request.EndDate)
	if err != nil {
		c.Echo().Logger.Errorf("Error occurred : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	// baseQuery := `INSERT INTO isubnb.reservation_home(id, user_id, home_id, date, number_of_people, is_deleted) `
...
func postApiV1ReservationActivity(c echo.Context) error {
	...
	getReservationActivityQuery := `SELECT ra.id FROM isubnb.activity a JOIN isubnb.reservation_activity ra ON a.id = ra.activity_id WHERE a.id = ? AND ra.date = ?`
	err = db.Select(&reservationActivityIdList, getReservationActivityQuery, request.ActivityId, request.Date)
	if err != nil {
		c.Echo().Logger.Errorf("Error occurred : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
